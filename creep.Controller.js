...
let REPORTING_TICK_INTERVAL = 120; 
let REPORTING_TICK_INTERVAL_SHORT = 30;
let QUOTA_CREEP_HARVESTER = 7;
let QUOTA_CREEP_HARVESTER_MIN = 3;
let QUOTA_CREEP_UPGRADER = 4;
let QUOTA_CREEP_BUILDER = 1;
...
let creepSpawner = {
    ...
    spawnCreep: function (targetRoom) {
        ...
        if  (spawner.room.energyAvailable >= MIN_ENERGY_FOR_PRODUCTION || 
                (   (harvesters.length <= QUOTA_CREEP_HARVESTER_MIN) && 
...
//                        let creepName = Game.spawns.Spawn1.createCreep([TOUGH,TOUGH,WORK,CARRY,CARRY,CARRY,MOVE], undefined, {role: 'harvester'});
                ...
                if(upgraders.length < QUOTA_CREEP_UPGRADER) {
                    ...
                    let creepName = Game.spawns.Spawn1.createCreep([WORK,WORK,WORK,WORK,WORK,WORK,WORK,WORK,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE], undefined, {role: 'upgrader'});
                    if (typeof(creepName) === 'string') {
                    ...
                    if(builders.length < QUOTA_CREEP_BUILDER) {
                        ...
                        if(harvesters.length > QUOTA_CREEP_HARVESTER-2 && harvesters.length < QUOTA_CREEP_HARVESTER) { // spawn excess harvesters as dedicated repar bots
                            let creepName = Game.spawns.Spawn1.createCreep([WORK,WORK,WORK,WORK,WORK,WORK,WORK,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,MOVE,MOVE,MOVE,MOVE,MOVE], undefined, {role: 'repair'});
                            if (typeof(creepName) === 'string') {
